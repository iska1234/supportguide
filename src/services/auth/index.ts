...
const createMainUser = async (email: string, name: string, lastname: string, password: string, phone: string) => {
	try {

		const response = await axios.post<NetworkSuccess<MainUserRegister>>(`${USER_END_POINT}`, {
...
const changeTenant = async (name: string) => {
	try {
		...
		const response = await axios.post<NetworkSuccess>(`${END_POINT}/changeTenant`, {
			name
		},{
				headers: {
